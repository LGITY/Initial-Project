...
class CircleSlider: UIView {
    ...
    // colors
    var circle_color = UIColor(red:0.90, green:0.90, blue:0.90, alpha:1.0)
    var touch_color = UIColor(red:0.11, green:0.17, blue:0.27, alpha:1.0)
    var trail_color = UIColor(red:0.13, green:0.70, blue:1.00, alpha:1.0)
    // for ease of use
    ...
    private var inner_circle: CAShapeLayer!
    ...
    func makeSlider() -> Bool {
        ...
        outer_circle = UIBezierPath(arcCenter: circle_center, radius: (circle_diameter / 2) + touch_tolerance,
                                    startAngle: 0, endAngle: CGFloat(Double.pi * 2), clockwise: true)
        inner_circle = CAShapeLayer()
        let inner_circle2 = UIBezierPath(arcCenter: circle_center, radius: (circle_diameter / 2) - touch_tolerance,
                                    startAngle: 0, endAngle: CGFloat(Double.pi * 2), clockwise: true)
        inner_circle.path = inner_circle2.cgPath
        inner_circle.strokeColor = UIColor(red:0.11, green:0.17, blue:0.27, alpha:1.0).cgColor
        
        let circlePoint = getPointOnCircle(forPoint: CGPoint(x: circle_center.x, y: circle_center.y - (circle_diameter/2)))
        touch_circle = UIBezierPath(arcCenter: circlePoint, radius: touch_diameter / 2, startAngle: 0, endAngle: CGFloat(Double.pi * 2), clockwise: true)
...
//    }
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let touch = touches.first!.location(in: self)
        if touch.x > circle_center.x - 5 && touch.x < circle_center.x + 5 && touch.y < circle_center.y  {
            CreateEvent3.text.text = "∞"
        }
        else {
            if getPointOnCircle(forPoint: touch).x > circle_center.x {
                 CreateEvent3.text.text = String(Int((atan((getPointOnCircle(forPoint: touch).y - circle_center.y)/(circle_diameter/2)) + (CGFloat.pi/4))/((CGFloat.pi*2)/100))+1)
            }
            else {
                CreateEvent3.text.text = String(Int((atan((circle_center.y - getPointOnCircle(forPoint: touch).y)/(circle_diameter/2)) + (CGFloat.pi/4))/((CGFloat.pi*2)/100))+26)
            }
        }
        
        
//        if tracker != 0 && tracker % 5 == 0 {
//            CreateEvent3.text.text = String(tracker/5)
//        }
        if outer_circle.contains(touch) && inner_circle.contains(touch) == false {
...
//            }
        ...
        touch_trail = UIBezierPath(arcCenter: circle_center, radius: (circle_diameter / 2),
                                   startAngle: -CGFloat.pi/2, endAngle: rad, clockwise: true)
    ...
    private func getRads(withOffset offset: CGFloat, fromRad rad: CGFloat) -> (CGFloat, CGFloat) {
        ...
        if rad <= 0 {
            leftRad = rad - offset
            if leftRad! < -(CGFloat.pi) { leftRad! = CGFloat.pi - (CGFloat.pi + leftRad!) }
        } else {
            leftRad = rad - offset
        }
        if rad <= 0 {
            ...
            rightRad = rad + offset
            if rightRad! > CGFloat.pi
            { rightRad = -(CGFloat.pi) + (rightRad! - CGFloat.pi) }
        }
